    ...
    defp copy_dir(source, dest, assigns) do
      ...
      try do
        ...
        :ignore_file_and_directory ->
          File.rm_rf!(dest)
          Mix.shell().info([:green, "- deleting", :reset, " #{dest} ", :faint, :cyan, "(it isn't needed)"])
      end
    end
    ...
    defp copy_and_expand(source, dest, assigns) do
      try do
        try do
          ...
          _ ->
            Mix.shell().info([:green, "- copying", :reset, " #{dest} ", :faint, :cyan, "(binary file?)"])
            File.cp!(source, dest)
        end
        ...
        :ignore_file ->
          Mix.shell().info([:green, "- ignoring", :reset, " #{dest} ", :faint, :cyan, "(it isn't needed)"])
      end
    end
